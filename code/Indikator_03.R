...
Erreichbarkeit <- function(origins, destinations) {
  ...
  grid_with_times <- polygrid100 %>%
    filter(if ("Bevölkerung" %in% names(.)) Bevölkerung != 0 else TRUE) %>%
    left_join(shortest_travel_times %>%
                dplyr::select(from_id, to_id, travel_time_p01), by = c("id" = "from_id")) %>%
    ...
    rename("GN_ziel" = GN, "KN_ziel" = KN)
  
  st_write(grid_with_times, here("output/indikator_03.gpkg"), layer = paste0(timestamp, "_nrw_gtfs_polygrid_100m_", ort$GN, "_", poi_type, "_", modes_filename, "_", max_trip_duration,"min"), append = FALSE)
}
...
Erreichbarkeit_nofiltering <- function(origins, destinations) {
  ...
  grid_with_times <- polygrid100 %>%
    ...
    #dplyr::select(!c("ART", "STAND", "KN7stellig")) %>%
    rename("GN_ziel" = GN, "KN_ziel" = parent_KN)
...
catchment_fun <- function(grid, overlay){
  ...
  total_pop <- st_join(overlay, grid, join = st_contains) %>%
    ...
    summarize(
      ...
      bevölkerung_sum = sum(Bevölkerung, na.rm = TRUE
      )
    )
  ...
  catched_pop <- st_join(overlay, grid_catched %>%
    ...
    summarize(
      count = n(),  # Count of intersecting features
      bevölkerung_catched_sum = sum(Bevölkerung, na.rm = TRUE)
    )
  ...
  uncatched_pop <- st_join(overlay, grid_uncatched %>%
    ...
    summarize(
      count = n(),  # Count of intersecting features
      bevölkerung_uncatched_sum = sum(Bevölkerung, na.rm = TRUE)
    )
...
pois_df <- pois_fun(pois = get(poi_type))
...
zensusstats <- i3full %>%
  st_drop_geometry() %>%
  group_by(origin_KN) %>%
  mutate(pop_sum = sum(Bevölkerung)) %>%
...
group_by(origin_KN) %>%
  mutate("sum_orignotdest" = sum(Bevölkerung)) %>%
  dplyr::select(9:12) %>%
  distinct() %>%
...
i3all_noaccess_stats <- i3all_noaccess %>%
  st_drop_geometry() %>%
  group_by(origin_KN) %>%
  mutate(uncatched = sum(Bevölkerung)) %>%
