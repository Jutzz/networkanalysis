...
zentraleOrte <- st_transform(st_read(here("geodata/poi.gpkg"), "zentrale_orte"), crs = crs(zensus_grid))
...
ttm <- Erreichbarkeit_nofiltering(origins = pois_df[10,], destinations = zensus_grid_df, departure = as.POSIXct("2026-05-12 12:00:00"))
...
for(d in days[days > as_date("2026-06-02")]){
  for (h in 8:17) {
