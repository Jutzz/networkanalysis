...
grid_tt_gz <- data.table::rbindlist(
  lapply(
    processed_files,
    function(f) {
      read_fst(f, as.data.table = TRUE)[
        zentralitaet == "Grundzentrum"
      ]
    }
  )
) %>%
...
write_fst(grid_tt_gz, "output/i3_shortest_hourly/full/to_gz.fst")
rm(grid_tt_gz)
gc()

grid_tt_oz <- data.table::rbindlist(
  lapply(
    processed_files,
    function(f) {
      read_fst(f, as.data.table = TRUE)[
        zentralitaet == "Oberzentrum"
      ]
    }
  )
) %>%
...
write_fst(grid_tt_oz, "output/i3_shortest_hourly/full/to_oz.fst")
rm(grid_tt_oz)
gc()
