...
#Uncomment to process new fahrplaene_gesamtdeutschland. Downloaded feed into de_fix.
gtfs_feed <- tidytransit::read_gtfs(paste0("feeds/", feed_date, "_fahrplaene_gesamtdeutschland_gtfs.zip"))
unzip(paste0("feeds/", feed_date, "_fahrplaene_gesamtdeutschland_gtfs.zip"), files = c("routes.txt", "stops.txt"), exdir = paste0("feeds/", feed_date, "_fahrplaene_gesamtdeutschland_gtfs"))
area <- st_read(here("geodata/dvg1nw.gpkg"), "regierungsbezirk_kln")
#area <- st_read(here("geodata/base_data/dvg1_EPSG25832_Shape/dvg1bld_nw.shp"))
de_gtfs_area <- filter_feed_by_area(gtfs_feed, area)
tidytransit::write_gtfs(de_gtfs_area, paste0("feeds/de_fix/de_gtfs_05-26_",area_name,".zip"))

#filter_feed_by_area doesnt keep parent_stops and routes in transfers, so this is handled manually
de_gtfs_stops <- read_csv(paste0("feeds/", feed_date,"_fahrplaene_gesamtdeutschland_gtfs/stops.txt"))
de_gtfs_routes <- read_csv(paste0("feeds/", feed_date,"_fahrplaene_gesamtdeutschland_gtfs/routes.txt"))
area_feed <- tidytransit::read_gtfs(paste0("feeds/de_fix/de_gtfs_05-26_",area_name,".zip"))
...
tidytransit::write_gtfs(area_feed, paste0("feeds/de_gtfs_05-26_",area_name,".zip"))
